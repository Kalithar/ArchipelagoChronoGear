...
from Locations2 import location_table, event_location_table, ChronoGearLocation
...
def create_and_connect_regions(world: ChronoGearWorld):
    ...
    for region in darkness_regions:
        if region == steel_steel:
            world_darkness.connect(region, "World of Darkness to Steel on Steel", lambda state: state.has_all_counts({region.name: 1, "Laplus' Shackle": world.options.steel_on_steel_shackle_requirement}, world.player))
        elif region == zstm:
            world_darkness.connect(region, "World of Darkness to Zero Seconds to Midnight", lambda state: state.has_all_counts({region.name: 1, "Laplus' Shackle": world.options.zero_seconds_to_midnight_shackle_requirement}, world.player))
        else:
    ...
    for name, data in location_table:
        ...
        if region == "Intermission":
            if world.options.intermission_world_unlocks == False:
                ...
                set_rule(world.get_location(name), lambda state: state.can_reach_location("Ganmo's Grand Finale - Clear Level") and state.can_reach_location("Wrath of Nature - Clear Level"))
            else:
                world.get_region("The Space Between Worlds").add_locations({name, data.id}, ChronoGearLocation)
        #Levels that require Chrono Gear
        #Case Closed unlock
        #Battle of Bell Town unlock
        #Atlantis unlock
        #Space Between Worlds unlock
        else:
